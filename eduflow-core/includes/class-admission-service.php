...
final class EduFlow_Admission_Service {
	...
	private static function clean( $input ) {
		...
		return array( 'student_name'=>sanitize_text_field( $input['student_name'] ?? '' ), 'mobile'=>$mobile, 'email'=>$email ?: null, 'course'=>sanitize_text_field( $input['course'] ?? '' ), 'level'=>sanitize_text_field( $input['level'] ?? '' ), 'class_type'=>$class, 'preferred_timing'=>sanitize_text_field( $input['preferred_timing'] ?? '' ), 'admission_date'=>self::date( $input['admission_date'] ?? '' ), 'total_fee'=>$total, 'amount_paid'=>$paid, 'pending_amount'=>max( 0, $total - $paid ), 'payment_status'=>$paid >= $total && $total > 0 ? 'paid' : ( $paid > 0 ? 'partial' : 'pending' ), 'notes'=>sanitize_textarea_field( $input['notes'] ?? '' ) );
	}
